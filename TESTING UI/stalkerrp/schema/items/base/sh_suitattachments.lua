...
function ITEM:PopulateTooltip(tooltip)
    if (!self.entity) then
        ix.util.PropertyDesc(tooltip, "Можно прикрепить к броне", Color(64, 224, 208))
    end
	...
	OnRun = function(item, data)
		...
		if targetAttach then
			if #targetAttach >= target.miscslots then
				client:Notify("Ваша броня не имеет свободного места.")
				return false
			end
		end
		...
		client:Notify("Установлено: "..item.name..". Предмет: "..target.name)

ITEM.name = "Сырая оленина"
ITEM.description = "Продукт с высоким содержанием белка и минералов, подходит для использования в различных блюдах."
ITEM.longdesc = "Сырое мясо оленя - это богатый источник белка, минералов и витаминов группы B, но требует правильной обработки перед употреблением, чтобы избежать рисков заболевания инфекциями."
...
ITEM.functions.use = {
    ...
    OnRun = function(item)
        ...
        ix.util.PlayerPerformBlackScreenAction(item.player, "Употребление...", 4, function(player) 
			ix.chat.Send(player, "me", "на грани рвоты, давясь от запаха, прожёвывает каждый кусочек сырого мяса оленя.")
		end)

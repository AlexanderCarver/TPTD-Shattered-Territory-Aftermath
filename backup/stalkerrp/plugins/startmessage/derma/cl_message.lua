...
hook.Add("PopulateHelpMenu", "ixHelpMenu", function(tabs)
	...
	tabs["FAQ"] = function(container)
	    ...
	    container.BetaText = container:Add("DLabel")
        container.BetaText:SetFont("ixGuideSmallFont")
        container.BetaText:SetText("\n\n2) Где я появился?\nВаш персонаж появился в админ-зоне. Здесь безопасно, несмотря на сырость и темноту... Вам нужно пройти ролевое одобрение, чтобы Ваш персонаж соответствовал игровой вселенной, а потом получить стартовый подарок от администрации и доброе напутствие. Думаешь, что второе необязательно? Поверь: Территория опасна, так как тварей (которые преследуют тебя всю карту), там различных хватает. Тебе сейчас нужно отписаться в OOC-чат (//), чтобы к прилетел администратор и объяснил механики игры, после чего продумать своего персонажа (хотя бы внешность). Отсюда иначе не выйти.")
        container.BetaText:Dock(TOP)
        container.BetaText:SetTextColor(color_white)
        container.BetaText:SetExpensiveShadow(1, color_black)
        container.BetaText:SetWrap(true)
        ...
        container.BetaText = container:Add("DLabel")
        container.BetaText:SetFont("ixGuideSmallFont")
        container.BetaText:SetText("\n\n3) О чём сервер?\nСервер повсящён хорошему РП, историям персонажей в стиле текстового РП (взаимоотношениям персонажей, грубо говоря), и хорошо оформленной атмосфере ужасных 90-ых. Здесь Вы сможете найти многие механики: начиная от психики со здоровьем и заканчивая иным подходом к ивентам, который предполагает бросок кубика даже при хождении по крыше (тебя ничто не защитит от злого ивентолога, который ищет способы умертвить персонажа). По большей части, технически сервер даже правильнее назвать StalkerRP, так как тут тоже есть фракции и артефакты с обособленной территорией, где происходят разные ужасы, но в совершенно ином, самописном мрачном мире, который отличается мрачной атмосферой неопределённости и позволяет ощутить тяжесть отрешённого человека, что живёт в России 90-ых...")
        container.BetaText:Dock(TOP)
        container.BetaText:SetTextColor(color_white)
        container.BetaText:SetExpensiveShadow(1, color_black)
        container.BetaText:SetWrap(true)
        ...
        container.BetaText = container:Add("DLabel")
        container.BetaText:SetFont("ixGuideSmallFont")
        container.BetaText:SetText("\n\n4) Дискорд?/Коллекция?/Донат?\nВсё лежит во вкладке помощи. Если не нашли ответ на свой вопрос - обратись к администрации через тикет (@) или OOC-чат (//).")
        container.BetaText:Dock(TOP)
        container.BetaText:SetTextColor(color_white)
        container.BetaText:SetExpensiveShadow(1, color_black)
        container.BetaText:SetWrap(true)

...
ix.config.Add("PsyHealthRecoverTime", 30, "Сколько секунд потребуется на восстановление одного поинта психики?", nil, {
...
ix.command.Add("CharSetPsyHealth", {
	description = "Установить процент психики персонажа. Только попробуй абузить - дам пизды."
	adminOnly = true,
	arguments = {
		ix.type.string,
		ix.type.number,
	...
	OnRun = function(self, client, target, psyhealth)
		...
		if client == target then
            client:Notify("Вы установили своё психическое здоровье на "..psyhealth.."%.")
        else
            client:Notify("Вы установили психическое здоровье "..target:Name().." на "..psyhealth.."%.")
            target:Notify("Администратор "..client:Name().." установил Ваше психическое здоровье на "..psyhealth.."%.")
        end

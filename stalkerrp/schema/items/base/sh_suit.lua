...
ITEM.ballisticareas = {"  Голова:", "  Торс:", "  Паховая область:", "  Руки:", "  Ноги:", "  Аномалии:", "  Радиация:"}
...
function ITEM:GetDescription()
	...
	if self:GetData("durability", nil) then
		duradesc = "\n\n".."Прочность: " .. math.floor(self:GetData("durability", 100)) .. "%"
	end
	...
	function ITEM:PopulateTooltip(tooltip)
		if !self.entity then
			...
			if self:GetData("setSkin", self.newSkin) then
				...
				skintitle:SetText("\nКамуфляж: "..self:GetData("setSkin", self.newSkin))
				skintitle:SizeToContents()
			end

			local duratitle = tooltip:AddRowAfter("skintitle", "duratitle")
			duratitle:SetText("Прочность: " .. math.floor(self:GetData("durability", 100)) .. "%")
			duratitle:SizeToContents()

			ix.util.PropertyDesc2(tooltip, "Защитный костюм", Color(64, 224, 208), Material("vgui/ui/stalker/weaponupgrades/handling.png"))

			if self.isGasmask then
	        	ix.util.PropertyDesc2(tooltip, "Противогаз", Color(64, 224, 208), Material("vgui/ui/stalker/weaponupgrades/handling.png"))
	        end
	        
	        if self.isHelmet then
	        	ix.util.PropertyDesc2(tooltip, "Защита головы", Color(64, 224, 208), Material("vgui/ui/stalker/weaponupgrades/handling.png"))
	        end
...
ITEM.functions.EquipUn = { -- sorry, for name order.
	...
	OnRun = function(item)
		...
		ix.util.PlayerPerformBlackScreenAction(item.player, "Вы раздеваетесь...", 5, function(player) 
		end)
...
ITEM.functions.Equip = {
	...
	OnRun = function(item)
		...
		for _, v in pairs(items) do
			if (v.id != item.id) then
				...
				if (itemTable.isBodyArmor and v.outfitCategory == item.outfitCategory and itemTable:GetData("equip")) then
					item.player:Notify("На Вас уже надет костюм!")
		...
		ix.util.PlayerPerformBlackScreenAction(item.player, "Вы надеваете экипировку...", 6, function(player) 
		end)

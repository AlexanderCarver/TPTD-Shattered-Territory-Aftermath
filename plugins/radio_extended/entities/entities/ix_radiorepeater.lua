...
ENT.Type = "anim"
ENT.PrintName = "Дубликатор каналов рации"
ENT.Category = "Helix"
ENT.Spawnable = true
ENT.AdminOnly = true
...
if (SERVER) then
	...
	net.Receive("ixRepeaterMenu", function(length)
		...
		if theEntity:GetClass() == "ix_radiorepeater" then
			...
			Derma_Query("Выберите операцию для выполнения", "Управление радиопередатчиком",
			"Изменение частот входного сигнала ("..oldInputFreq.." MHz)",function()
				Derma_StringRequest("Частота", "Текущая входная частота: "..oldInputFreq.." MHz".."\nНа что бы Вы хотели установить новую частоту?", oldInputFreq, function(text)
				repeater("I"..text)
				--ix.command.Send("SetFreq", text)
				end)
			end,
			"Изменение частот входного сигнала ("..oldOutputFreq.." MHz)",function()
				Derma_StringRequest("Частота", "Текущая исходная частота: "..oldOutputFreq.." MHz".."\nНа что бы Вы хотели установить новую частоту?", oldOutputFreq, function(text)
				repeater("O"..text)
				--ix.command.Send("SetFreq", text)
				end)
			end,
			"Активировать ток", function()

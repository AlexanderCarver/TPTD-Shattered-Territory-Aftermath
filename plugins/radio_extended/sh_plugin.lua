...
local radioanon = {"Неизвестный", "Неизвестный", "Неизвестный", "Неизвестный"}
...
if (CLIENT) then
	...
	netstream.Hook("Channel", function(names)
		...
		local quer = Derma_Query("Нажмите чтобы выбрать канал\nПравый клик чтобы сменить канал", "Выбор канала",
		...
		function ch1panel:DoRightClick()
			Derma_StringRequest("Название канала", "Какое название Вы хотите установить каналу?\nНажмите пробел, чтобы установить по умолчанию", names[1], function(text)
				if text == " " then 
					setTheName("1","CH1")
				else
					setTheName("1",text)
		...
		function ch2panel:DoRightClick()
			Derma_StringRequest("Название канала", "Какое название Вы хотите установить каналу?\nНажмите пробел, чтобы установить по умолчанию", names[2], function(text)
				if text == " " then 
					setTheName("2","CH2")
				else
					setTheName("2",text)
		...
		function ch3panel:DoRightClick()
			Derma_StringRequest("Название канала", "Какое название Вы хотите установить каналу?\nНажмите пробел, чтобы установить по умолчанию", names[3], function(text)
				if text == " " then 
					setTheName("3","CH3")
				else
					setTheName("3",text)
		...
		function ch4panel:DoRightClick()
			Derma_StringRequest("Название канала", "Какое название Вы хотите установить каналу?\nНажмите пробел, чтобы установить по умолчанию", names[4], function(text)
				if text == " " then 
					setTheName("4","CH4")
				else
					setTheName("4",text)
	...
	netstream.Hook("FrequencyMenu", function(oldFreqs)
		...
		Derma_Query("Выберите операцию для выполнения", "Дуплексный Регулятор Частоты",
		"Изменение частот передачи ("..oldTransmit.." MHz)",function()
			Derma_StringRequest("Частота", "Текущая частота передачи: "..oldTransmit.." MHz".."\nНа что бы Вы хотели установить новую частоту?", oldTransmit, function(text)
			--repeater("I"..text)
			ix.command.Send("SetFreq", text)
			end)
		end,
		"Изменение частот передачи ("..oldListen.." MHz)",function()
			Derma_StringRequest("Частота", "Текущая частота передачи: "..oldListen.." MHz".."\nНа что бы Вы хотели установить новую частоту?", oldListen, function(text)
			--repeater("O"..text)
...
function PLUGIN:OverwriteClasses()
	...
	do
		...
		CLASS.format = "%s передает в рацию: \"%s\""
	...
	do
		...
		CLASS.format = "%s кричит в рацию: \"%s\""
		CLASS.mult = 0.15
	...
	-- LAST ONE FOR WHISPERING
		...
		CLASS.format = "%s шепчет в рацию: \"%s\""
		CLASS.mult = -0.3
	...
	do
		...
		function COMMAND:OnRun(client, message)
			...
			local call
			local defCall
			local names = {"Неизвестный", "Неизвестный", "Неизвестный", "Неизвестный"}
		...
		function COMMAND:OnRun(client, frequency)
			...
			if (self:TableLength(radios) < 1) then
				...
				if itemTable.walkietalkie then
					client:Notify("Вы не можете напрямую изменить частоту этого радио.")
				elseif itemTable:GetData("duplex",itemTable.duplex) and !notSame then
					client:Notify("Ваши передающие и приемные частоты не могут быть одинаковыми!")
				elseif (active == 1) then
					if string.find(frequency, "^%d%d%d%.%d$") then
						...
						client:Notify(string.format("Вы установили свою радиочастоту на %s MHz.", frequency))
					end
				...
				elseif (numEnabled > 1) then
					client:Notify("Активируйте один из Ваших радиоприемников, чтобы установить частоту.")
					-- for k, v in ipairs(radios) do
						-- if (v:GetData("enabled", false)) then
							...
							-- if (string.find(frequency, "^%d%d%d%.%d$")) then
								...
								-- if (active < 1) then 
									-- client:Notify(string.format("Вы установили свою радиочастоту на %s.", frequency))
								-- end
								-- active = active+1
							-- end
			...
			if (self:TableLength(radios) < 1) then
				...
				elseif (active == 1 and itemTable:GetData("duplex",itemTable.duplex)) then
					if string.find(listenfrequency, "^%d%d%d%.%d$") then
						...
						client:Notify(string.format("Вы установили свою частоту приема на %s MHz.", listenfrequency))
					end
				...
				elseif !itemTable:GetData("duplex",itemTable.duplex) then
					client:Notify("Ни один из Ваших радиоприемников не способен на такое действие.")
				end
			end
		end
	...
	do
		...
		function COMMAND:OnRun(client, chan)
			...
			if !validchan then
				client:Notify("Недопустимая спецификация канала.")
				return
			end
			...
			if (self:TableLength(radios) < 1) then
				...
				if itemTable then
					...
					client:Notify(string.format("Вы настроили свой радиоканал на %s.", chan))
				else
					client:Notify("У Вас нет активного радио!")
				end
			end
	...
	do
		...
		function COMMAND:OnRun(client, chan)
			...
			elseif (string.len(chanName) > maxChars) then
				client:Notify("Ваше название канала слишком длинное!")
				return
			end
			...
			if (self:TableLength(radios) < 1) then
				...
				if itemTable then
					...
					client:Notify(string.format("Вы установили канал связи %s на %s.", chan,chanName))
				else
					client:Notify("У Вас нет активного радио!")
				end
			end
	...
	do
		...
		function COMMAND:OnRun(client, target, callsign)
			...
			if (ix.config.Get("enableCallsigns")) then
				...
				client:Notify(string.format("Вы установили позывной %s на %s",target:GetName(),callsign))
				target:GetPlayer():Notify(string.format("Ваш позывной был установлен на %s с помощью %s",callsign,client:Name()))
			else
				client:Notify("В настоящее время позывные отключены!")
			end
		end
		...
		function COMMAND:OnRun(client)
			...
			if (ix.config.Get("allowBroadcast",true)) then
				...
				if (self:TableLength(radios) > 0) then
						...
						if (v:GetData("enabled",false) and v:GetData("active",true)) then
							...
							if (self:CheckLegal(bAllowed, v) )  then
								...
								if v:GetData("broadcast") then
									...
									local show = string.format("Теперь вы вещаете по всем каналам %s.",nowalkie and " на частоте "..v:GetData("frequency","100.0").." MHz" or "")
									client:NotifyLocalized(show)
								else
									client:NotifyLocalized("Вы больше не вещаете по всем каналам.")
								end
								break
							end
						end
					...
					if !found then
						client:NotifyLocalized("Ни один из ваших радиоприемников в настоящее время не активен.")
					elseif !legal then
						client:NotifyLocalized("Ваше радио не способно вещать.")
					end
				else
					client:NotifyLocalized("У Вас нет радио!")
				end
			
			else
				client:NotifyLocalized("Радиовещание было отключено.")
			end
		end
				...
				for k,v in pairs(radios) do
					if (v:GetData("enabled",false) and v:GetData("active",false)) then
						...
						if v:GetData("scanning",false) then
							local nowalkie = !v.walkietalkie
							local preShow = string.format("На частоте %s MHz", v:GetData("duplex",v.duplex) and v:GetData("listenfrequency","100.0") or v:GetData("frequency","100.0"))
							local show = string.format("Теперь вы слушаете все каналы %s.",nowalkie and preShow or "")
							client:NotifyLocalized(show)
						else
							client:NotifyLocalized("Вы больше не слушаете все каналы.")
						end
						break
						--end
					end
	...
	do
		...
		function COMMAND:OnRun(client)
			...
			if (#radioEnts > 0) then
				...
				client:Notify("Успешно удалены все радиопередатчики.")
			else
				client:Notify("Никаких ретрансляторов радио, чтобы удалить!")
			end
		end

...
ENT.Type = "anim"
ENT.PrintName = "Corporate Forcefield"
ENT.Category = "Tower Corporation: Others"
ENT.Spawnable = true
ENT.AdminOnly = true
...
local MODE_ALLOW_PASS1 = 2
local MODE_ALLOW_PASS2 = 3
local MODE_ALLOW_PASS3 = 4
local MODE_ALLOW_PASS4 = 5
local MODE_ALLOW_NONE = 6
...
if (SERVER) then
	...
	local MODES = {
		...
		},
		{
			function(client)
				...
				if (character and character:GetInventory() and !character:GetInventory():HasItem("pass1")) then
					return true
				else
					return false
				end
			end,
			"Только сотрудники c L-1 Security Pass."
		},
		{
			function(client)
				local character = client:GetCharacter()

				if (character and character:GetInventory() and !character:GetInventory():HasItem("pass2")) then
					return true
				else
					return false
				end
			end,
			"Только сотрудники c L-2 Security Pass."
		},
		{
			function(client)
				local character = client:GetCharacter()

				if (character and character:GetInventory() and !character:GetInventory():HasItem("pass3")) then
					return true
				else
					return false
				end
			end,
			"Только сотрудники c L-3 Security Pass."
		},
		{
			function(client)
				local character = client:GetCharacter()

				if (character and character:GetInventory() and !character:GetInventory():HasItem("pass4")) then
					return true
				else
					return false
				end
			end,
			"Только сотрудники c L-4 Security Pass."
		},
		{
			function(client)
				return true
			end,
			"Только сотрудники служб безопасности."
		}
	}
	...
	function ENT:Use(activator)
		...
		if (activator:IsCombine()) then
			...
			activator:ChatPrint("Изменение режима: "..MODES[self:GetMode()][2])

...
function ENT:Detonate(impact)
    ...
    if self:WaterLevel() > 0 then
        ...
        self:EmitSound("weapons/arc9_doi/panzerfaust/water/panzerfaust_water_detonate_0" .. math.random(1, 3) .. ".wav", 100)
        self:EmitSound("weapons/arc9_doi/panzerfaust/water/panzerfaust_water_detonate_dist_0" .. math.random(1, 3) .. ".wav", 130)
    else
        ...
        // Overpressure radius
        util.BlastDamage(self, IsValid(self:GetOwner()) and self:GetOwner() or self, self:GetPos(), 64, 500)
        // Shrapnel radius
        util.BlastDamage(self, IsValid(self:GetOwner()) and self:GetOwner() or self, self:GetPos(), 512, 80)

        self:EmitSound("weapons/arc9_doi/panzerfaust/panzerfaust_detonate_0" .. math.random(1, 3) .. ".wav", 125)
        self:EmitSound("weapons/arc9_doi/panzerfaust/panzerfaust_detonate_dist_0" .. math.random(1, 3) .. ".wav", 150)
        self:EmitSound("weapons/arc9_doi/panzerfaust/panzerfaust_detonate_far_dist_0" .. math.random(1, 3) .. ".wav", 175)
    end
